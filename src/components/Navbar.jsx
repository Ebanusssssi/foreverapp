...
const Navbar = () => {
  ...
  return (
    <nav className="flex items-center justify-between py-5 font-medium">
      ...
      <div className="flex flex-wrap justify-end items-center gap-6 pl-5 sm:pl-0">
        <img 
      ...
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white/30 backdrop-blur-[30px] transition-all z-50 ${visible ? 'w-full' : 'w-0'}`}>
        <div className="flex flex-col text-gray-600">
          <div onClick={()=>setVisible(false)} className="flex items-center gap-4 p-3 cursor-pointer border-b">
            ...
            <p className='text-black/40'>Back</p>
          </div>
          <NavLink onClick={()=>setVisible(false)} to='/' className='py-2 border-b text-center leading-[3rem]'>
            HOME
          </NavLink>
          <NavLink onClick={()=>setVisible(false)} to='/collection' className='py-2 border-b text-center leading-[3rem]'>
            COLLECTION
          </NavLink>
          <NavLink onClick={()=>setVisible(false)} to='/about' className='py-2 border-b text-center leading-[3rem]'>
            ABOUT
          </NavLink>
          <NavLink onClick={()=>setVisible(false)} to='/contact' className='py-2 border-b text-center leading-[3rem]'>
            CONTACT
          </NavLink>
        </div>
      </div>
